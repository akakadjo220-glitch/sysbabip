-- ============================================================
-- Migration: 20260320_accounting_safe_commissions.sql
-- Description:
-- Stores commission_rate and commission_amount in transactions table
-- to ensure historical stability (Accounting Lock).
-- ============================================================
...
-- 2. Initialiser les taux par défaut dans platform_settings si absents
...
-- 3. Backfill : Figer les commissions sur l'historique actuel
-- On utilise le taux actuel de l'événement comme référence pour le passé.
UPDATE transactions t
SET 
  commission_rate = COALESCE(e.commission_rate, 8.0),
  commission_amount = ROUND(t.amount * (COALESCE(e.commission_rate, 8.0) / 100.0), 0)
FROM events e
WHERE t.event_id = e.id AND (t.commission_rate IS NULL OR t.commission_amount IS NULL);

-- 4. get_admin_stats (RPC)
...
BEGIN
  -- Revenue & Commission (Digital) via colonnes gelées
  SELECT 
...
BEGIN
  -- Gross & Commission via colonnes gelées
  SELECT 
    COALESCE(SUM(t.amount), 0),
    COALESCE(SUM(t.commission_amount), 0)
...
-- 6. Mise à jour des Vues pour utiliser commission_amount
...
SELECT 
  ...
  (er.revenue - er.commission) - COALESCE(ea.advance_deducted, 0) AS payout_amount
FROM event_rev er
...
SELECT 
  ...
  er.evt_status,
  er.gross,
  -- Note: comm_rate devient indicatif ici (basé sur le total)
  CASE WHEN er.gross > 0 THEN (er.commission / er.gross * 100) ELSE 0 END AS comm_rate,
  er.adv_rate,
  er.commission AS comm,
  (er.gross - er.commission) AS base_net,
  er.tickets_sold,
  CASE WHEN (er.gross - er.commission) > 50000 THEN 'En attente' ELSE 'Transféré' END AS payout_status,
  COALESCE(ea.advance_amt, 0) AS advance_amt,
  ea.advance_status,
  (er.gross * (er.adv_rate / 100.0)) AS max_advance,
  (er.gross - er.commission) - CASE WHEN ea.advance_status IN ('paid', 'approved', 'pending') THEN ea.advance_amt ELSE 0 END AS remaining_payout
FROM event_rev er
LEFT JOIN event_advances ea ON er.event_id = ea.event_id;
